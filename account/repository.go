...
func (r *postgressRepository) Close() { // here we have function close for the struct postrgressRepository
...
func (r *postgressRepository) GetAccountByID(ctx context.Context, id string) (*Account, error) {
	row := r.db.QueryRowContext(ctx, "SELECT id, name FROM accounts WHERE id = $1", id) // this queries the database to get the account by id and scans the result into the account struct
...
func (r *postgressRepository) ListAccounts(ctx context.Context, skip uint64, take uint64) ([]Account, error) {
	...
	for rows.Next() {       // here now we will access each and every id from the row and will add it to the list of accounts
		...
		if err := rows.Scan(&a.ID, &a.Name); err != nil { // will scan and will access eaach and every enrty from the row of a Account
			accounts = append(accounts, *a) // appending the account to the list of accounts
		}
	}
