...
func main() {
	...
	retry.ForeverSleep(2*time.Second, func(_ int) (err error) {
		r, err = account.NewPostgressRepository(cfg.DatabaseURL) // here we are going to create a new Postgress account Repo which will be the connection to our database and URL
		if err != nil {
			log.Println(err)
		}
		return
